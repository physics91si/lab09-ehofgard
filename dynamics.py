...
def init_molecule():
    ...
    connecting them"""
    mol = molecule.Molecule(np.array([0.2,0.2]),np.array([0.8,0.8]),1,2,1,0.5)
    return mol
...
def time_step(dt, mol):
    ...
    force = mol.get_force()
    mol.p1.vel = -mol.p1.vel+(force/mol.p1.m)*(0.5*dt)
    mol.p2.vel = mol.p2.vel+(force/mol.p2.m)*(0.5*dt)
